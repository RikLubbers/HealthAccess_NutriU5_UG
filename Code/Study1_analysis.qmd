...
required_packages <- c(
  ...
  "languageserver", "httpgd", "miceadds", "ggmice", "howManyImputations", "gridExtra"
)
...
region_plot = ggplot(DHS_children_sub, aes(x = reorder(V024, travel_time, median), y = travel_time, fill = V024)) +
  ...
  ggtitle("Distribution of Travel Time by Region for DHS Household Clusters") +
  scale_fill_manual(values = pastel_colors) + # Use the custom color scale
  theme_minimal() +
  guides(fill = FALSE) +
  theme(
    plot.title = element_text(hjust = 0.5, size = 20, face = "bold"), # Increase size for plot title
    axis.title = element_text(size = 14), # Increase size for axis titles
    axis.text.x = element_text(angle = 45, hjust = 1, size = 12, face = "bold"), # Increase size for x axis text
    axis.text.y = element_text(size = 12), # Increase size for y axis text
    plot.caption = element_text(size = 10) # Increase size for caption
  )
