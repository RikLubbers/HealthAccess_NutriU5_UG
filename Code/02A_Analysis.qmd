...
required_packages <- c(
    "data.table", "modelsummary", "expss", "ggmice", "ggplot2", "haven", "howManyImputations", "mice",
    "NCmisc", "readxl", "stats", "tidyr", "utils", "performance", " tidyverse"
)
...
# Simple model comparison with MICE
```{r simple model comparison}
# Simple model comparison with MICE using only the variables of interest
## Stunting, Wasting, and Underweight
## Note: The models are not weighted by the sampling weights (V005) in this analysis
glm_stunted_tt_only <- with(
    DHS_children_sub_multimp,
    lm(HW5 ~ travel_time, )
)

glm_underweight_tt_only <- with(
    DHS_children_sub_multimp,
    lm(HW8 ~ travel_time, )
)

glm_wasted_tt_only <- with(
    DHS_children_sub_multimp,
    lm(HW11 ~ travel_time, )
)

## Compare the models
models_tt_only <- modelsummary(list(pool(glm_stunted_tt_only), pool(glm_wasted_tt_only), pool(glm_underweight_tt_only)),
    stars = TRUE,
    coef_rename = TRUE,
...
models_tt_only
# Compare the models excluding variables with >50% missingness
glm_stunted_reduced <- with(
    DHS_children_sub_multimp,
    lm(HW5 ~ B4 + BORD + M19 + B11 + H11 + H22 + V481 + V729 + V505 + V191 + V212 + V445 + V714 + V113 + V136 + H7 + travel_time, )
)

glm_underweight_reduced <- with(
    DHS_children_sub_multimp,
    lm(HW8 ~ B4 + BORD + M19 + B11 + H11 + H22 + V481 + V729 + V505 + V191 + V212 + V445 + V714 + V113 + V136 + H7 + travel_time, )
)

glm_wasted_reduced <- with(
    DHS_children_sub_multimp,
    lm(HW11 ~ B4 + BORD + M19 + B11 + H11 + H22 + V481 + V729 + V505 + V191 + V212 + V445 + V714 + V113 + V136 + H7 + travel_time, )
)

# Model summary and checking
modelsummary(pool(glm_stunted_reduced),
    stars = TRUE,
    coef_rename = TRUE
)

modelsummary(pool(glm_underweight_reduced),
    stars = TRUE,
    coef_rename = TRUE
)

modelsummary(pool(glm_wasted_reduced),
    stars = TRUE,
    coef_rename = TRUE
)

# Compare models
models_reduced <- modelsummary(list(pool(glm_stunted_reduced), pool(glm_wasted_reduced), pool(glm_underweight_reduced)),
    stars = TRUE,
    coef_rename = TRUE,
    col.names = c("", "Stunting", "Wasting", "Underweight"),
    coef_omit = "^(?!trav)",
